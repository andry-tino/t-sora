...
### Let's try to run it!
What if we wanted to see the effect of what we are building? Well, we are actually ready to write the code which will render the whole scene. To do so, just add the following lines to `index.js`:
...
The logic for animating everything is inside `sora.utils.performAnimation` and it just needs to be passed the animation array:
...
**Important** From now on, we will add lines where the comments is suggesting.

What can we see by the way? I bet you get a black screen :) That is normal. Let's move on and understand why.
...
If we try to run this as we did before, unfortunately we would still be looking at a black screen. That is because there is no object that our lights are actually hitting. We need to put something in the scene.
...
Let's try to run it! Now we can finally see something! A universe filled with sky!
...
Same as when we focused on the Moon! Let's try to run it, we will see the planet.

However, we want to add an animation to the Earth: we want the Earth to rotate. So we add the following lines:
...
If we try to run it again, we will see the Earth moving.
...
If we try to run it, we will see a very thin blue layer above the planet surface. As you can see it is still not enough, so let's move on.
...
As you can see, this atmosphere will contain the previous one as we make it a big bigger. If we try to run again, we will see the whole atmosphere in place!
...
The code for this unction is very simple: we just call `sora.utils.createCloudMesh` which has the logic for generating clouds. We have imported `utils.js` in `index.js` (together with `earth.js`), so we can assume the code in `utils.js` will be available here as well.
...
Try to run in the browser, we can see the clouds and the Earth below moving around. The problem is that the clouds should move, and they should move faster than the rotation speed of the Earth.
...
We have set the speed to `1/8` which is higher than `1/32` (Earth's rotation speed). If we try to run again, we will see the clouds moving faster than the Earth!

## Adding mouse interaction
The last step is adding mouse interaction. When the planets have been rendered on the scene, we want the camera we just created to move around the planet depending on the movements of the mouse on the browser.

In `index.js`, the first thing we do is defining a variable which will contain the mouse coordinates:

{% highlight javascript %}
var mouse = { x: 0, y: 0 };
{% endhighlight %}

We set initial values to `{ x: 0, y: 0 }`, but these will be updated to store some information relating to the position of the mouse everytime that the mouse moves on the browser. To do this, we add the following lines:
...
The call to `document.addEventListener` register a function to an event, in this case `mousemove`, this event is fired everytime the mouse moves on the browser window. So everytime this happens, we update the value of variable `mouse`. The values we calculate are coordinates normalized by the window size. It means that we will get values between 0 and 1 where 0 means the position is on the axis, and 1 means on the farthest point.

We also make a subtraction by 0.5, this shifts the axis origin from the top left corner to the center of the screen. So we will now get values from -0.5 to 0.5. Like this we have basically abstracted our calculation to a point where, no matter the browser window size, we will always deal with the same range of values.

The next step is reading these values reported in variable `mouse`. To do so, write down the following lines of code:
...
As we have done for all animations, we need to push the function responsible for animating inside array `animations`. Once we complete the code, this array will be traversed and each function called. 

In this code we are basically making the camera move in the direction of the mouse, factors 5 and 3 that we used basically define the speed. The last line always keeps the target of the camera on the Earth. It means that we will always keep looking at the Earth no matter what.

## We are done!
Try to run now, you will see the 2 planets as you move your mouse around. Congratulations, your animation is done!
