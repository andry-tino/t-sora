...
As we have done for all animations, we need to push the function responsible for animating inside array `onRenderFcts`. Once we complete the code, this array will be traversed and each function called. 
...
## Finalizing it
We are ready for the very last step. We first need to ensure that we render the scene everytime according to our frame rate. To do so, just add the following lines to `index.js`:
